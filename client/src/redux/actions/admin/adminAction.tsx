...
export const adminLogout = createAsyncThunk('user/adminLogout', async(_, {rejectWithValue}) => {
  try {
    const { data } = await axios.get(`http://localhost:3001/api/admin/logout`, config)
    return data
  } catch (error) {
    const axiosError = error as AxiosError<MyApiError>;
    return handleError(axiosError, rejectWithValue);
